...
// Complete the minimumBribes function below.
    static void minimumBribes(int[] q) {
        ...
        for(int i = q.length -1; i >= 0; i--){
            //A bigger number can only move forward maximum of 2 times
            //Thus, if a bigger number occupies a position greater than itself + 2
            //it is impossible.
            //However, a number can constantly be forced backwards 
            //Where each position can "swap" with him. Thus a smaller number can 
            //move back as much as possible.
            //e.g. value 8 at index 4 is impossible
            if(q[i] - (i+1) > 2){
                System.out.println("Too chaotic");
                return;
            }
            //why Math.max(0, q[i] -2)?
            //Math.max so that it will not go out of bound
            //q[i] - 2 
            //Minus 1 is for correcting it to the correct index
            //Another minus is because it is possible for the value it swapped
            //to move forward by 1. 
